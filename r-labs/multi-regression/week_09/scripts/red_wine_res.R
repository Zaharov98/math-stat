...
прогpreноз.lg = predict.lm(res.02, wine.03)
...
cook.d <-  cooks.distance(res.01)
plot(cook.d,ylab="Cooks distances")


abline(h=1, col = "red")
abline(h=1/174, col = "blue")



#  сравнение остатков в разных группах

#  2 способа задать номер сезона

seas.1 <- rep(0, 174)

for (i in 1:12)
{
  seas.1 <- seas.1 + i * ser.g.02[1:174, i+2]
}

seas.2 <- rep(1:12, 16)[1:174]

seas.2


boxplot(rstudent(res.01)~seas.2)
...
#  library(car)
#  durbinWatsonTest(res.01, method="normal")
...
прогноз.lg = predict.lm(res.01, wine.02)
